...
class TutorSignuoVuewController:UIViewController{
	...
	func uniqueUsername (username: String? , allUser: Array<String>) -> Bool{ //Ensures that use enters a unique username
				...
				FIRAuth.auth()?.createUser(withEmail: emailTextField.text!, password: passwordTextField.text!) { user,error in
					if self.confirmTextField.text! == self.passwordTextField.text!  && self.passwordTextField.text!.characters.count >= 6
					{
						...
						else
						{
							...
							FIRDatabase.database().reference(withPath: "Users/\(FIRAuth.auth()!.currentUser!.uid)").updateChildValues(["uid": "Tutor:\(FIRAuth.auth()!.currentUser!.uid)", "displayName": username, "Email": email])
						}
					}
					else
					{
